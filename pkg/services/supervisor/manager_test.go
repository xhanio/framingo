...
func TestStatsHealthcheck(t *testing.T) {
	t.Run("returns nil when healthy", func(t *testing.T) {
		s := &entity.SupervisorStats{Name: "svc", Initialized: true}
		assert.NoError(t, s.Healthcheck())
	})

	t.Run("reports stopped", func(t *testing.T) {
		s := &entity.SupervisorStats{Name: "svc", Stopped: true}
		err := s.Healthcheck()
		require.Error(t, err)
	...
	t.Run("reports init error", func(t *testing.T) {
		s := &entity.SupervisorStats{Name: "svc", InitializationErr: fmt.Errorf("init boom")}
		err := s.Healthcheck()
		require.Error(t, err)
	...
	t.Run("reports start error", func(t *testing.T) {
		s := &entity.SupervisorStats{Name: "svc", StartErr: fmt.Errorf("start boom")}
		err := s.Healthcheck()
		require.Error(t, err)
